...
int max_sum_subarrayDQ(const vector<int> &v, int low, int high){
    if(high == low){
        return v[low];
    }
    int mid = (high + low)/2;
    
    int left_subarray = max_sum_subarrayDQ(v, low, mid);
    int right_subarray = max_sum_subarrayDQ(v, mid+1, high);
    int crossing_subarray = max_crossing_subarray(v, low, mid, high); 
}

int main(){
   vector<int>j =  {-2, -3, 4, -1, 1, 3, 4, -6, -5, 2, 6, -2, 1, 5, -3}; 
   int max_sum = max_sum_subarrayDQ(j, 0, j.size() - 1); 
